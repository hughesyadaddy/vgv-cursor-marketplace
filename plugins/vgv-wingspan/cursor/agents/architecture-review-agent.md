...
description: 'Validates project architecture against VGV standards post-implementation. Use after writing code to verify layer separation, state management correctness, dependency direction, and package structure.'
model: inherit
---

<!-- CURSOR_VGV_AGENT -->
> **Dual-host port:** Cursor subagent inheriting parent tools
> and MCP servers. Cursor has no per-agent tool allowlist —
> upstream `tools:` is mapped to `readonly` when no write
> tools were listed. Use **AskQuestion** on Cursor or
> **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) on Claude Code for option lists.
> On Cursor never output `/clear` or `/new-chat`.
> Model: `inherit` (mapped from upstream `inherit`).
> Companion skills: elements-of-style.
...
**Before reviewing, detect the project's tech stack:** Read the project's AGENTS.md, dependency manifests, linting configuration, and directory structure to determine the specific tools and frameworks in use. Apply VGV's architectural standards to whatever stack the project uses.
...
1. **Check your available skills.** Installed plugins expose their skills to you directly — scan your available-skills list for ones whose descriptions match the code under review, and load the relevant ones by reading its SKILL.md. Only invoke skills that appear in your list; never guess names.
2. **Glob project-local skills** the plugin system does not manage: `.cursor/skills/**/SKILL.md`. Read each match's frontmatter (`name`, `description`) and the full content of any whose domain matches.
...
1. Read the project's AGENTS.md and architecture documentation for defined layers
