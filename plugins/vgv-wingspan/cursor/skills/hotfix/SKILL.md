...
- Invoke Cursor subagent codebase-review-agent("Locate the code responsible for this bug. Focus narrowly on the symptom described — do not survey the entire codebase. Bug: <bug_description>")
...
If the root cause is unclear after exploration, use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to ask the user for additional context before proceeding.
...
**Otherwise:** infer a branch name in the format `hotfix/<kebab-slug>` where the slug is derived from the bug description (max 60 characters total). Use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to confirm:
...
**If >5 files, multiple layers, or new abstractions needed:** warn the user and use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code): (1) Proceed with hotfix, (2) Switch to `/plan`.
