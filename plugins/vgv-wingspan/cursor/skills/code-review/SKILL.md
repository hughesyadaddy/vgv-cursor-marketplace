...
<!-- CURSOR_CODE_REVIEW_NOTE -->
> Cursor built-in `/review` is Bugbot/Security only.
> This skill is `/code-review` (Wingspan review).
...
- **If `SCOPE=default`**: tell the user "You're on `<CURRENT_BRANCH>`. No branch diff
  available." Use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code): "What would you like to review?" with options:
...
This skill is advisory — take no action until the user picks one. Use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to
