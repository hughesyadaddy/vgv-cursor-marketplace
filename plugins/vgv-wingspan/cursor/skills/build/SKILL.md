...
| Empty | Multiple | **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code): list each with summary, ask which to use |
...
**Commit autonomy:** decide once how this build commits, and carry the choice through the whole run. Honor a saved preference if one exists (Claude memory or the user's personal settings); otherwise use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code):
...
Offer to save the choice to Claude memory (a personal preference) so future builds skip this question. Save it as the user's own preference — never write it to the project's AGENTS.md, since committing this is a per-developer choice, not a repo convention.
...
**Otherwise**, summarize scope to the user, then use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to confirm:
...
- **More phases remain, auto-commit mode** → use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code):
  1. **Clear context and continue (Recommended)**: build the next phase in a fresh window. Follow the [same-chat handoff](references/cursor-same-chat-handoff.md) with `<NEXT_SKILL>` = `build`, `<DOC_PATH>` = this plan's path, and `<NEXT_ACTION>` = "the next phase". Then **stop**.
...
| Agent | Report name |
| ----- | ----------- |
| **Cursor subagent vgv-review-agent** | `vgv-review` |
| **Cursor subagent architecture-review-agent** | `architecture-review` |
| **Cursor subagent test-quality-review-agent** | `test-quality-review` |
| **Cursor subagent code-simplicity-review-agent** | `code-simplicity-review` |
| **Cursor subagent pr-readiness-review-agent** | `pr-readiness-review` |
...
- **No such preference** → use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) before anything leaves the machine:
...
Use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to present options:
