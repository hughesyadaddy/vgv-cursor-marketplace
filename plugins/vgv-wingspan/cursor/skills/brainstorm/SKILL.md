...
**If new project**, use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code): "This sounds like a new project. Where would you like to work?"
...
**If clear:** Use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to suggest proceeding directly to planning.
...
- Invoke Cursor subagent codebase-review-agent("Understand existing patterns related to: <feature_description>")

Focus on: similar features, established patterns, AGENTS.md guidance.

#### 1.2. Collaborative conversation

Use the **host structured question tool (AskQuestion on Cursor; AskUserQuestion on Claude Code)** to ask questions one at a time. The tool automatically provides an "Other" option for free-text input — never add your own catch-all option (e.g., "Something else", "None of the above").
...
Use **host structured question tool (AskQuestion on Cursor; AskUserQuestion on Claude Code)** to ask which approach the user prefers.
...
Use **host structured question tool (AskQuestion on Cursor; AskUserQuestion on Claude Code)** to consider next steps:

**Question**: "Brainstorm complete! What would you like to do next?"

**Options:**
1. **Plan now (Recommended)**: continue in this chat and plan
2. **Review and refine approach:** improve the document using structured review
