...
- **Cursor subagent code-simplicity-review-agent** — review the plan for simplicity and clarity; the implementation should be as straightforward as possible while still meeting every requirement.
- **Cursor subagent vgv-review-agent** — review the plan for adherence to Very Good Engineering practices and project conventions.
- **Cursor subagent plan-splitting-agent** — assess plan scope and report whether the work is too large for a single reviewable PR.
...
| Split recommended | Use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to choose how to act (below). |
