...
1. **Read referenced files**: Read any specific files called out in the brainstorm as relevant context.
...
| Strong local context (good patterns, AGENTS.md guidance, clear user intent) | Skip external research |
...
- **Cursor subagent official-docs-research-agent**: Fetches and synthesizes official documentation for relevant frameworks, libraries, and APIs.
- **Cursor subagent best-practices-research-agent**: Researches and synthesizes best practices for the project's technology stack, following VGV conventions first, then official documentation, and finally industry standards.
...
- Invoke Cursor subagent user-flow-analysis-agent(feature_description, research_findings)
...
Surface every rejected or rewritten criterion to the user with **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) before writing the plan file — do not silently change the spec they approved.
...
- Run `git rev-parse --abbrev-ref HEAD`. If the current branch is a base branch (`main`, `master`, or `develop`), use **AskQuestion** (Cursor) / **AskUserQuestion** (Claude Code) to offer creating a feature branch — `git checkout -b <type>/<kebab-topic>`, name under 60 characters — before writing. If already on a feature branch, continue without prompting.
...
After the review completes, use the **host structured question tool (AskQuestion on Cursor; AskUserQuestion on Claude Code)** and present the following options:
