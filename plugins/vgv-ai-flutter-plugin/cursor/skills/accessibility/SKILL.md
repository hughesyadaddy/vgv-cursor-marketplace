...
Use `AskQuestion (Cursor; AskUserQuestion on Claude Code)` to ask:
...
Use `AskQuestion (Cursor; AskUserQuestion on Claude Code)` (multi-select if available, otherwise one question with a comma-separated reply) to ask:
...
1. **Touch Targets and Dragging Alternatives** — WCAG 2.2 2.5.8 minimum (24 CSS px) at AA, 2.5.5 enhanced (44 CSS px) at AAA, VGV recommended 48 dp, plus 2.5.7 dragging alternatives.

1. **Focus and Keyboard Navigation** — Operability, traversal order, dialog focus trapping, focus indicators, plus 2.4.11 / 2.4.12 focus-not-obscured.

1. **Color Contrast** — Text and UI component ratios at the selected level's threshold (table below).

1. **Text Scaling** — No fixed-height text containers, no clamped text scaling. Cap simulations at 2x on Android and Web, 3x on iOS.

1. **Animation and Motion** — `disableAnimations` gating across `AnimationController`, `Hero`, `AnimatedContainer`, `PageRouteBuilder`. No content flashing above 3 Hz at AA, zero flashing at AAA.

1. **Forms, Authentication, and Help** — `autofillHints` (1.3.5, 3.3.7), accessible authentication (3.3.8 / 3.3.9), consistent placement of help mechanisms (3.2.6).
...
After delivering the report, use `AskQuestion (Cursor; AskUserQuestion on Claude Code)`:
