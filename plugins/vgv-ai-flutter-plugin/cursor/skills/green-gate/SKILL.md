...
allowed-tools: Bash Read Glob Grep Edit Write analyze_files (dart MCP) dart_format (dart MCP) test (very-good-cli MCP)
model: sonnet
effort: medium
---
...
- **Use MCP tools, never the Bash CLI, for gates** — analyze via
  `analyze_files (dart MCP)`, format via `dart_format (dart MCP)`, test and
  coverage via `test (very-good-cli MCP)`. The Bash test path
...
1. **Analyze** — run `analyze_files (dart MCP)` with `applyFixes: true`. If any
   errors remain, this is the active gate. Fix, record the fingerprint, go to
   step 7.
1. **Format** — run `dart_format (dart MCP)` on the package root. It reformats
   the whole package in place. If it reports changed files, the gate is now
   green for the next round.
1. **Test** — only if analyze is green this round. Run `test (very-good-cli MCP)`
   with the coverage parameters from **Test Gate**. If tests fail, fix, record
   the fingerprint, go to step 7.
1. **Coverage** — only if tests pass this round. The MCP `min_coverage` result is
...
1. **Exit** — if all four gates are green in *this same iteration*, confirm
   success with the observed numbers and stop. This is the only exit-green path.
1. **Re-verify** — increment the iteration counter, recompute the failure
...
1. **Every gate is re-evaluated every round; green is never cached.** Exit
...
- Run `analyze_files (dart MCP)` with `applyFixes: true` so quick fixes are
...
- **Hook interplay** — the optional afterFileEdit `analyze.sh` hook fires on every
...
- Run `dart_format (dart MCP)` on the package root each round. It runs
...
- **Why format is a real gate, not just the hook** — the optional afterFileEdit `format.sh`
...
- Run `test (very-good-cli MCP)` with `coverage: true`, `min_coverage: <target>`,
...
- **Shared package-root discovery** — walk for `pubspec.yaml` files. The
  ...
  `test (very-good-cli MCP) --recursive` (`recursive: true`) covers.
